...
export function MarketScreen() {
  ...
  async function pickMedia() {
    ...
    if ((videos.length && result.assets.length !== 1) || result.assets.some((asset) => (asset.fileSize ?? 0) > 5 * 1024 * 1024)) return Toast.show({ type: 'error', text1: 'Market', text2: 'Choose up to two images or one video, max 5 MB each.' });
  ...
  async function publish() {
    ...
    try {
      ...
      const media = selectedMedia.length ? await Promise.all(selectedMedia.map((asset) => uploadMarketMedia({ byteSize: asset.fileSize ?? 1, clientPostId, contentType: asset.mimeType ?? (asset.type === 'video' ? 'video/mp4' : 'image/jpeg'), fileName: asset.fileName ?? `${randomUUID()}.${asset.type === 'video' ? 'mp4' : 'jpg'}`, uri: asset.uri }))) : undefined;
...
function MarketCard({ post, onLike, onComments, onChat, onCall, onEdit, onDelete }: { post: MarketPost; onLike: () => Promise<void>; onComments: () => void; onChat: () => Promise<void>; onCall: () => Promise<void>; onEdit: () => void; onDelete: () => Promise<void> }) {
  ...
  return <View className="overflow-hidden rounded-2xl border-2 border-black bg-white shadow-sm">
    ...
    <View className="mx-4 mb-3 flex-row flex-wrap gap-2"><Badge text={`${post.price.toLocaleString()} ${post.currency}`} /><Badge text={`Qty ${post.quantity}`} /><Badge text={`⌖ ${post.city}`} /></View>
    {post.media?.map((item) => item.kind === 'video' ? <MarketVideo key={item.id} uri={item.url} /> : <Image key={item.id} source={{ uri: item.url }} className={`w-full ${post.media?.length === 2 ? 'h-56' : 'h-80'}`} contentFit="cover" />)}
    <View className="flex-row items-center border-t border-black/10 p-2"><Action label={`♥ ${post.likeCount}`} active={post.likedByViewer} onPress={onLike} /><Action label={`💬 ${post.commentCount}`} onPress={async () => onComments()} />{!post.ownedByViewer && <Action label="☎" onPress={onCall} />}<Pressable onPress={() => void onChat()} className="mx-1 rounded-full bg-black px-5 py-3"><Text className="font-black text-white">Chat</Text></Pressable></View>
  </View>;
}
