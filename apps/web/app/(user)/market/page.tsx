...
const EMPTY_FIELDS: MarketPostFields = { content: '', price: 0, currency: 'USD', quantity: 1, city: '' };
    ...
    <main ref={feedRef} className="min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto max-w-2xl space-y-3 p-3">
        ...
        {posts.map((post) => <article key={post.id} className="overflow-hidden rounded-2xl border-2 border-black bg-white shadow-[5px_6px_0_#111]">
          ...
          <div className="mx-4 mb-3 flex flex-wrap gap-2"><Badge><b>{post.price.toLocaleString()} {post.currency}</b></Badge><Badge><Package size={13} /> {post.quantity}</Badge><Badge><MapPin size={13} /> {post.city}</Badge></div>
...
function Composer({ fields, mediaFiles, busy, onChange, onFiles, onPublish }: { fields: MarketPostFields; mediaFiles: File[]; busy: boolean; onChange: (fields: MarketPostFields) => void; onFiles: (files: File[]) => void; onPublish: () => void }) { return <section className="space-y-2 rounded-2xl border border-black/10 bg-white p-4"><textarea value={fields.content} onChange={(event) => onChange({ ...fields, content: event.target.value })} placeholder="What are you selling?" maxLength={4000} className="min-h-24 w-full resize-none rounded-xl border border-black/15 p-3 outline-none" /><div className="grid grid-cols-3 gap-2"><NumberInput value={fields.price || ''} placeholder="Price" onChange={(price) => onChange({ ...fields, price })} /><NumberInput value={fields.quantity} placeholder="Quantity" onChange={(quantity) => onChange({ ...fields, quantity: Math.floor(quantity) })} /><input value={fields.city} onChange={(event) => onChange({ ...fields, city: event.target.value })} placeholder="City" maxLength={100} className="min-w-0 rounded-xl border border-black/15 px-3" /></div>{mediaFiles.length > 0 && <div className="flex gap-2 overflow-x-auto">{mediaFiles.map((file, index) => <FilePreview key={`${file.name}-${index}`} file={file} onRemove={() => onFiles(mediaFiles.filter((_, itemIndex) => itemIndex !== index))} />)}</div>}<div className="flex justify-between"><label className="cursor-pointer rounded-xl border border-black/15 px-4 py-3 text-xs font-black">📎 {mediaFiles.length ? `${mediaFiles.length} selected` : 'Media'}<input className="hidden" type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/quicktime,video/webm" onChange={(event) => { const files = [...(event.target.files ?? [])]; const videos = files.filter((file) => file.type.startsWith('video/')); if (files.some((file) => file.size > 5 * 1024 * 1024) || (videos.length && files.length !== 1) || (!videos.length && files.length > 2)) return toast.error('Choose up to two images or one video, max 5 MB each.'); onFiles(files); }} /></label><button disabled={busy} onClick={onPublish} className="rounded-xl bg-black px-6 py-3 font-black text-white disabled:opacity-40">{busy ? 'Posting…' : 'Post'}</button></div></section>; }
...
function EditModal({ post, onClose, onSave }: { post?: MarketPost; onClose: () => void; onSave: (postId: string, fields: MarketPostFields) => Promise<void> }) { const [fields, setFields] = useState(EMPTY_FIELDS); useEffect(() => { if (post) setFields({ content: post.content, price: post.price, currency: post.currency, quantity: post.quantity, city: post.city }); }, [post]); if (!post) return null; return <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-5"><div className="w-full max-w-lg space-y-3 rounded-[24px] bg-white p-5"><div className="flex justify-between"><h2 className="text-lg font-black">Edit listing</h2><button onClick={onClose}><X /></button></div><textarea value={fields.content} onChange={(event) => setFields({ ...fields, content: event.target.value })} className="min-h-28 w-full rounded-xl border border-black/15 p-3" /><div className="grid grid-cols-3 gap-2"><NumberInput value={fields.price} placeholder="Price" onChange={(price) => setFields({ ...fields, price })} /><NumberInput value={fields.quantity} placeholder="Quantity" onChange={(quantity) => setFields({ ...fields, quantity: Math.floor(quantity) })} /><input value={fields.city} onChange={(event) => setFields({ ...fields, city: event.target.value })} className="min-w-0 rounded-xl border border-black/15 px-3" /></div><button onClick={() => { if (validFields(fields)) void onSave(post.id, fields); }} className="w-full rounded-xl bg-black p-3 font-black text-white">Save</button></div></div>; }
