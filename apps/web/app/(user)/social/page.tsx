...
import { Bell, Heart, MessageCircle, Pencil, Send, UserRound, X } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useCallback, useEffect, useRef, useState } from 'react';
import toast from 'react-hot-toast';

import { sessionStorage } from '@/app/api/session-storage';
import { createSocialComment, createSocialPost, deleteSocialComment, deleteSocialPost, getSocialProfile, listSocialAlerts, listSocialComments, listSocialPosts, markSocialAlertsRead, reportSocialPost, toggleSocialCamp, toggleSocialLike, updateSocialPost, updateSocialProfile, uploadSocialMedia, type SocialAlert, type SocialComment, type SocialPost, type SocialProfile, type SocialVisibility } from '@/app/api/social';
  ...
  return <div className="flex h-full flex-col bg-[#e7e7e9] text-[#171717]">
    <header className="flex h-14 shrink-0 items-center justify-between border-b border-black/10 bg-gradient-to-b from-white to-[#c9c9cb] px-4 shadow-sm">
      <h1 className="text-lg font-black">
        g
        <span className="text-[#c62828]">000</span>
        st
            ...
            <div className="mt-2 flex gap-2 overflow-x-auto">
              {mediaFiles.map((file, index) => (
                <MediaPreview
                  key={index}
                  file={file}
                  onRemove={() => setMediaFiles((current) => current.filter((_, i) => i !== index))}
                />
              ))}
            </div>
          )}
          <div className="mt-2 flex items-center gap-2">
            <label className="flex items-center gap-2 rounded-xl border border-black/10 px-3 py-2 text-xs font-bold">
              📎<input
                ...
                onChange={(event) => {
                  const files = [...(event.target.files ?? [])]; const videos = files.filter((file) => file.type.startsWith('video/')); if (files.some((file) => file.size > 5 * 1024 * 1024)) { toast.error('Each file must be 5 MB or smaller.'); event.target.value = ''; return; } if ((videos.length && files.length !== 1) || videos.length > 1 || (!videos.length && files.length > 2)) {
      ...
      {view === 'alerts' ?
        ...
        <div className="mx-auto max-w-2xl space-y-3 p-3">
          {view === 'mine' && profile &&
            <ProfileEditor
              profile={profile}
              onSave={async (value) => setProfile(await updateSocialProfile(value))}
            />
          }
          {posts.length === 0 &&
            <Empty
              text={view === 'mine' ? 'No posts yet — share something.'
                :
                'No posts yet.'
              }
            />
    ...
    <nav className="flex h-14 shrink-0 border-t border-black/15 bg-white/90">
      ...
      />
      <ViewButton
        active={view === 'mine'} onClick={() => setView('mine')}
        label="My Page"
      />
      <ViewButton
        active={view === 'alerts'} onClick={() => setView('alerts')}
        label="Alerts"
      />
    </nav>
...
function EditSocialPostModal({ post, onClose, onSave }: { post?: SocialPost; onClose: () => void; onSave: (postId: string, content: string) => Promise<void> }) { const [content, setContent] = useState(''); useEffect(() => { if (post) setContent(post.content); }, [post]); if (!post) return null; return <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-5"><div className="w-full max-w-lg space-y-3 rounded-[24px] bg-white p-5"><div className="flex justify-between"><h2 className="text-lg font-black">Edit post</h2><button onClick={onClose}><X /></button></div><textarea value={content} onChange={(event) => setContent(event.target.value)} maxLength={4000} className="min-h-32 w-full rounded-xl border border-black/15 p-3" /><button onClick={() => { const next = content.trim(); if (next) void onSave(post.id, next); }} className="w-full rounded-xl bg-black p-3 font-black text-white">Save</button></div></div>; }
