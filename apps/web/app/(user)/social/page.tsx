    ...
    <main className="min-h-0 flex-1 overflow-y-auto">
      ...
      {view === 'alerts' ? <Alerts alerts={alerts} /> : <div className="mx-auto max-w-2xl space-y-3 p-3">
        ...
        {posts.map((post) => <article key={post.id} className="overflow-hidden rounded-2xl border border-black/10 bg-white shadow-sm">
          <div className="flex items-center gap-3 p-4"><div className="grid h-10 w-10 place-items-center rounded-full bg-[#ddd]"><UserRound size={20} /></div><button className="min-w-0 flex-1 text-left" onClick={() => void openChat(post.ownerPublicId)}><div className="truncate font-black">{post.author.displayName}</div><div className="text-xs text-black/45">{new Date(post.createdAtMs).toLocaleString()}{post.editedAtMs ? ' · edited' : ''}</div></button><button className="text-xs font-black" onClick={async () => { if (post.ownedByViewer) { if (confirm('Delete this post?')) { await deleteSocialPost(post.id); setPosts((items) => items.filter((item) => item.id !== post.id)); } } else { await reportSocialPost(post.id, 'other'); toast.success('Report sent'); } }}>{post.ownedByViewer ? 'Delete' : 'Report'}</button></div>
