...
export type SocialProfile = { publicId: string; displayName?: string; showDisplayName: boolean; avatarUrl?: string; country?: string; age?: number; sex?: 'male' | 'female'; hobby?: string; bio?: string; updatedAtMs: number; campedByViewer: boolean };
...
export async function createSocialPost(clientPostId: string, content: string, visibility: SocialVisibility, media?: PendingSocialMedia[], sharedPostId?: string) { const { data } = await axios.post<{ post: SocialPost }>('/social/posts', { clientPostId, content, visibility, ...(media?.length ? { media } : {}), ...(sharedPostId ? { sharedPostId } : {}) }); return data.post; }
