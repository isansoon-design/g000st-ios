...
export async function listSocialPosts(cursor?: string, ownerId?: string) { const { data } = await axios.get<SocialPage<SocialPost>>('/social/posts', { params: { cursor, limit: 20, ownerId } }); return data; }
...
export async function updateSocialProfile(profile: Partial<Omit<SocialProfile, 'publicId' | 'updatedAtMs' | 'campedByViewer'>>) { const { data } = await axios.put<{ profile: SocialProfile }>('/social/profile', profile); return data.profile; }
